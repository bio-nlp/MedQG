...
if __name__ == "__main__":
    task_feedback = UsmleQgenFeedbackLgc( engine= "gpt-4",
        prompt_examples="data/prompt/usmle/feedback.jsonl", rubrics_path="data/prompt/usmle/reasoning_rubrics.jsonl"
    )
    ...
    f = open('data/inputs/dc_usmle_hum_complete_dat_top10.jsonl', "r")
...
# Reading from file
    ...
    for d in data:
        for diffi,usmle_q in d.items():
            try:
                context = '.'.join(usmle_q['question'].split('.')[:-1])
                question = usmle_q['question'].split('.')[-1]
                clinical_note = usmle_q['clinical_note']
                topic = usmle_q['topic']
                keypoint = usmle_q['keypoint']
                correct_answer = usmle_q['correct_answer']
                options = usmle_q['options']
                distractor_options = {k:options[k] for k in options.keys() if k != usmle_q['answer_idx']}
                distractor_options_text = ' '.join([f"{k} :" + f"{v}" for k,v in distractor_options.items()])
                options = ' '.join([f"{k} :" + f"{v}" for k,v in usmle_q['options'].items()])
                attempted_answer, reasoning = task_answer(context=context,question=question, options=options)
                print(context,question,clinical_note,distractor_options_text,attempted_answer)
                c_fb,c_score,q_fb,q_score,r_fb,r_score,ca_fb,ca_score,do_fb,do_score = task_feedback.__call__(clinical_note =  clinical_note, 
                            keypoint=  keypoint, 
                            topic=  topic, 
                            context=  context, 
                            question=  question, 
                            correct_answer=  correct_answer, 
                            distractor_options=  distractor_options_text,
                            attempted_answer=  attempted_answer,
                            reasoning=  reasoning)
                usmle_q['context_feedback'] = c_fb
                usmle_q['context_score'] = c_score
                usmle_q['question_feedback'] = q_fb
                usmle_q['question_score'] = q_score
                usmle_q['reasoning_feedback'] = r_fb
                usmle_q['reasoning_score'] = r_score
                usmle_q['correct_answer_feedback'] = ca_fb
                usmle_q['correct_answer_score'] = ca_score
                usmle_q['distractor_options_feedback'] = do_fb
                usmle_q['distractor_options_score'] = do_score
            except Exception as e:
                usmle_q['error']  = str(e)
                output_errpath = "data/inputs/dc_usmle_hum_complete_dat_top10_fberr.jsonl"
                with open(output_errpath, 'a+') as f:
                    json.dump(d,f)
                continue
        
        output_path = "data/inputs/dc_usmle_hum_complete_dat_top10_fb.jsonl"
